...
function DailyView() {
  ...
  const breathSteps = TOOLS.breathing.items[0].steps; // Box as default
...
export default function App() {
  ...
  // Reset to valid tab when switching roles
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.warm, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "0 28px" }}>
        {/* Tabs */}
        <div style={{ display: "flex", gap: 4, paddingTop: 20, paddingBottom: 20 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              padding: "8px 18px", borderRadius: 10, fontSize: 13, fontWeight: 600,
              background: tab === t.id ? C.teal : C.white,
              color: tab === t.id ? C.white : C.textMd,
